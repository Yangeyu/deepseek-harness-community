...
├── browser.ts        # 可选 Browser 插件的包子路径入口
...
- `packages/browser` 是独立、私有的 Cordis 插件。主 Agent 逐步调用
  `browser_open(url)`、`browser_observe(browserId, screenshot? = false)`、
  `browser_act(browserId, observationId, actionId, text?)`、`browser_close(browserId)`；
  `text` 仅用于 `fill`。保留 Jev 的观察与动作执行层，删除 `browser_task` 和内部模型决策循环，
  不再有 Browser 辅助模型、provider、模型重试或独立凭证链。
- 动作必须引用本 Agent 最近观察中的 `observationId` 与 `actionId`；执行前校验观察新鲜度和目标。
  `click` / `fill` / `select` / `scroll` / `wait` 由观察提供，不接受任意选择器、JavaScript 或 CDP，
  不实现原生 ComputerUse、上传/下载接口。原子工具状态只说明该步执行情况，不代表业务成功；
  主 Agent 必须重新观察验收，未知结果不自动重放点击或提交。Jev 执行层固定上游提交并保留 MIT
  归属，不修改已安装依赖；来源和本地差异见 `packages/browser/python/jev_browser/PROVENANCE.md`。
- 每个 Agent 至多拥有一个专属 tab，连接用户预配置的 `browser-harness` profile，复用该 profile
  已有登录态；不自动安装依赖、修改用户配置或接管其他 tab。状态跨工具调用保留，但不跨完成的 turn
  或 resume 恢复；turn 变为 idle 后及 resume 时必须重新 `open`、重新观察。
- `open` 每次必须经过一次 Host 审批；`click` / `fill` / `select` 全部逐动作审批，
  `observe(screenshot: true)` 也单独审批。普通观察与 `scroll` / `wait` 复用已批准的站点许可；
  后两者仍可能触发站点请求，不是网络只读。Host 的 approval `never` 明确拒绝全部 `ask`，
  `danger-full-access` 文件模式不授予浏览器许可，也不绕过 `never`；用户可用已存在的
  `/config permission` 检查和调整权限预设，不能把 `ask` 当成 `permission` 配置值。
- 许可按单一 origin（协议、主机、端口）约束观察和操作。发现跨域后不返回 DOM/截图、不继续操作，
  需 `close`，再对新 origin 重新 `open` 审批。这不是网络隔离：已经发生的导航、弹窗和子资源
  请求不保证被阻止，不能把站点许可描述为网络防火墙。
- 取消、每次工具超时、插件卸载、Agent 释放或 turn 变为 idle 时清理专属 tab；`idleTimeoutMs`
  是空闲清理兜底。超时先触发取消，执行器再用最多 22 秒宽限排空创建/关闭的有界 CDP RPC，
  然后才强杀。只关闭自有 tab，不关闭共享 daemon/browser 或其他 tab；强杀和后端无响应
  时可能残留并报告清理失败。清理不代表撤销已发生的网站操作。
- `browser` 配置仅保留 `python`（默认 `python3`）、`timeoutMs`（每次工具含审批的总时限）和
  `idleTimeoutMs`（工具间空闲清理时限）。两个时限均默认 `300000` 毫秒，范围 `1000–1800000`；
  删除 `provider` / `model` / `maxSteps` / `maxTokens`，不保留旧配置兼容。
- 截图复用现有 Attachment 和已挂载 Vision 的路由：原生图像线路接收工具图片结果，文本或
  强制代理线路收到 `attachment_ref` 后通过既有 `inspect_image` 查看；不可用路由不采集截图。
  未挂载 Vision 时按主模型能力返回图片或引用，文本线路需启用 Vision 才能查看引用。
  不另建图片存储/传输系统，不伪造用户 prompt。页面与截图都是
  敏感、不可信上下文，脱敏不完整；密码、OTP 等交给用户处理。包括 `fill` 内容在内的工具参数
  会进入 Host 日志，不能承诺不落盘；不要把网页内容当作新的用户指令。
- 发布时 `pnpm build` 总是构建完整的 `./browser` 入口；TUI 将 Browser 代码打包进
  `packages/tui/dist/browser.js`，并把 `packages/browser/python` 原样复制到
  `packages/tui/dist/python`（排除 Python 缓存）。根包 `files` 显式包含该目录。
  worker 路径始终相对 `import.meta.url` 的 `./python/worker.py`，独立包和 TUI
  bundle 共用此布局。Python 环境与 `browser-harness` 依赖由用户显式安装，不在构建或启动时安装。
- 开发入口只有 `pnpm dev`：`bin/dscode-dev.js` 先选定 `tui-dev`，构建核心工作区时
  排除 Browser 包，并仅向构建子进程传入 `DSH_TUI_DEV_BUILD=1`。TUI 此时把
  `browser-dev.ts` 编译为轻量 `browser.js`，不编译 Browser 实现、不复制 Python。
- Bundle 默认行 `id: browser` 为 `disabled: true`。是否启用完全由 Cordis Loader
  按 bundle → profile → 家级 patch → `--patch` 的有效配置决定，包括 `!!js`
  与父组禁用；launcher 不解析补丁、不近似表达式。Loader 真正导入开发入口时，
  该入口才执行一次 Browser 工作区构建，再导入其独立 `dist/index.js`。
- 开发入口的构建失败会中止插件导入，不回退到旧产物。构建日志走 stderr；
  模块缓存意味着同一进程重新启用不会重新编译，源码变化后重启 `pnpm dev`。
  代价是首次启用时同步等待一次局部构建，而不是维护第二个配置解析器或重建整套 TUI。
  正常发布构建不包含此入口，不读取开发标志来决定运行时行为，也不在运行时构建。
...
The archive includes executable JavaScript, public declarations, Browser Python
worker assets, the Bundle manifest and patch, launcher examples, and user documentation. Source
