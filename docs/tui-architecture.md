...
`TranscriptModel` 从同一 snapshot 同时投影正文 `items` 和待输入 `pendingInputs`，
统一按身份去重，不在 composer 再解释一遍队列。运行中提交的 steering/queued，以及
尚未进入对话的 Host inbox 消息，显示在输入框附近的独立预览区，不预先画成正式用户消息。
消费后保留原 steering/queueing 展示类别，直到 `user/message` 将它从预览原子交接到正文。
空闲普通提交仍立即显示正文回显；`QueuedInboxItem.localEcho` 只继承本地已知的展示归属，
让它在入列、消费时保持原位置，不因 Host 的运行状态变化在正文和预览间往返。
该标记不写入 Host、不改变投递模式，也不构成另一份执行队列。

`TranscriptProcess` 拥有正文和 `PendingInputPreview`，Shell layout 只分配屏幕区域：
预览位于原状态栏上方，Steering 在前、Queued 在后，各组保持消息顺序。标题使用现有
`secondary`，正文、箭头、隐藏数量和时机说明使用 `dim`，与正式用户消息的亮色和背景
区分；不新增主题或硬编码另一套颜色。每条正文是一行安全截断预览，多余消息显示隐藏
数量。Shell 根据终端高度的三分之一及剩余空间给出预算，优先保留原 composer 与对话空间；
`PendingInputPreview` 单独拥有最多 8 行的内容上限。窄窗口退化为分类计数；已有 Surface
占用输入区时不显示预览。投影项契约归 `TranscriptModel`，视图只消费投影，不反向定义模型契约。
本版不提供待输入消息展开、编辑或独立浏览入口，完整原文仍保存在原有 snapshot 中。
极窄时分类文案也可能截断；若原编辑器与附件本身已高于终端，预览会隐藏，但不在本次
修改中解决原 composer 的超高裁剪问题。

运行中 Vision 准备的消息也显示在预览区，分析进度使用原全局状态栏；不向正文插入无已入
会话消息对应的准备期工具卡。空闲 Vision 的即时正文回显和 activity、入会话后的 Vision
证据展示保持不变。`InputCoordinator.interruption()` 统一派生中断动作、目标身份和是否已
请求中断：优先取消交互，其次取消图片准备，最后中断 Session。按键执行、重复 Ctrl+C 退出
及状态栏提示消费同一份决策，状态栏不自行推断队列或取消优先级。只有当前动作确实会中断
Session 且 Host 队列中存在可续接 steering 时，Esc 提示才是 `esc: stop & send`；图片准备
期间保持普通中断提示，即使另有 steering 已入列也不宣称此时会重投。
spinner 仍只属于全局状态栏，`Ctrl+G` 回到最新提示仍在同一状态栏文本中。

同一输入从准备、入列到消费的预览高度保持稳定，空闲回显跨交接也保持位置；正式入会话
时允许正文增长及预览收缩产生一次正常布局变化，不以防抖或虚拟列表掩盖状态交接。
布局变化只夹取并保存手动阅读位置，不因当前位置恰好落到新视口底部而恢复尾随；明确向下
滚到底、`Ctrl+G` 或发送时的既有 follow 操作才恢复。测试覆盖 9/10/18/24/48 行、近尾部
取消预览后继续阅读、显式恢复跟随及 Surface 切换；该局部布局保障不等同于 P2 的全局语义
滚动锚点或窗口化。
