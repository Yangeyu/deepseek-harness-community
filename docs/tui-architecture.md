...
`context.spec.ts` 与 `learning-input.spec.ts` 验证预算、条件完整性和用户证据；服务测试验证真实 pre-step 投影与长回复后的候选调度。它们不证明总体回答质量提升。后续应以脱敏历史先生成记忆，再对未见过的任务进行固定模型对照；只有短索引与现有主题文件确实不足时，再评估语义文件定位或检索演进。
