...
`pnpm run release:check` is the only artifact acceptance entry point. Local
release-it runs it after updating the package version and before creating the
release commit or tag. Routine Linux and macOS CI runs the faster `pnpm check`;
the tag workflow repeats the artifact gate in the publishing environment. Each
gate builds and tests once, packs one archive, inspects its file boundary,
installs that archive into an isolated global npm prefix, and executes the
installed `dscode --version`. The tag workflow publishes the archive retained
by that gate; it never repacks a different payload.
