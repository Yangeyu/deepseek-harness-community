...
var RewindDialog = class {
	...
	render(width) {
		...
		const confirmation = wrapTextWithAnsi(this.theme.dim("Confirm you want to restore the workspace and conversation to the point before you sent this message:"), width);
...
function awaitingVisibleOutput(entries, showReasoning) {
	let awaiting = false;
	for (const entry of entries) {
		const event = entry.event;
		if (event.type === "user/message") {
			if (event.surfaceOp === "append" && event.data.source.kind === "user" && messageText(event.data.content, false).trim() !== "") awaiting = true;
			continue;
		}
		if (!awaiting) continue;
		if (event.type === "assistant/chunk") {
			const chunk = event.data.chunk;
			if ((chunk.type === "text-delta" || showReasoning && chunk.type === "reasoning-delta") && chunk.text.trim() !== "") awaiting = false;
			continue;
		}
...
function rowsFromState(state, theme, showReasoning, showDetails, maxToolOutputLines) {
	...
	if (localWorking || state.running && awaitingVisibleOutput(state.events, showReasoning)) rows.push({ loading: true });
	...
	render(width) {
		...
		for (const [index, row] of rows.entries()) {
			...
			if (row.prompt && row.body !== void 0) {
				...
				if (row.promptStatus !== void 0) lines.push(this.theme.dim(`  ${row.promptStatus}`));
				continue;
			}
...
var WorkspaceCheckpointStore = class {
	...
	async preview(sessionId, checkpointId) {
		const checkpoint = this.requireCheckpoints(sessionId).find((candidate) => candidate.id === checkpointId);
