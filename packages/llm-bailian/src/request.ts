...
async function serializeMessages(
  ...
  for (const message of options.messages) {
    ...
    if (message.role === 'assistant') {
      ...
      wire.push({ role: 'user', content: await userContent(message.content, attachments, policy, signal) })
    }
  }
...
export async function serializeRequest(
  ...
  return {
    model: model.id,
    messages: await serializeMessages(options, attachments, model.imageRequestPolicy, options.signal),
    stream: true,
