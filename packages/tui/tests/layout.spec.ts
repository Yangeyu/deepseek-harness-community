...
describe('ComposerAnchoredLayout', () => {
  ...
  it('follows the conversation tail and scrolls through the non-fixed header', () => {
    ...
    expect(layout.render(80).slice(0, 6).map(line => line.trimEnd()))
      .toEqual(['line 7', 'line 8', 'line 9', 'line 10', 'line 11', 'line 12'])
    expect(layout.followsTranscriptTail).toBe(true)

    expect(layout.scrollTranscript(-3)).toBe(true)
    expect(layout.render(80).slice(0, 6).map(line => line.trimEnd()))
      .toEqual(['line 4', 'line 5', 'line 6', 'line 7', 'line 8', 'line 9'])
    ...
    expect(layout.render(80).slice(0, 6).map(line => line.trimEnd()))
      .toEqual(['line 8', 'line 9', 'line 10', 'line 11', 'line 12', 'line 13'])

    expect(layout.scrollTranscript(-99)).toBe(true)
    expect(layout.render(80).slice(0, 3).map(line => line.trimEnd()))
  ...
  it('pages the transcript and maps screen rows to full transcript lines', () => {
    ...
    expect(layout.transcriptRowAt(0, 0)).toBe(-1)
    expect(layout.transcriptRowAt(1, 0)).toBe(0)
