...
import {
  ...
  parseMouseReport,
  resolveMouseAction,
} from '../../../src/infrastructure/terminal/decode-input.ts'
import {
  DISABLE_MOUSE_TRACKING,
  ENABLE_MOUSE_TRACKING,
} from '../../../src/presentation/shell/input/mouse-tracking.ts'
...
describe('parseMouseReport', () => {
  ...
  it('normalizes Surface navigation keys before shell resolution', () => {
    expect(decodeTerminalInput('j')).toMatchObject({ kind: 'key', key: 'j', phase: 'press' })
    expect(decodeTerminalInput('\u001b[C')).toMatchObject({ kind: 'key', key: 'right', phase: 'press' })
    expect(decodeTerminalInput('\r')).toMatchObject({ kind: 'key', key: 'enter', phase: 'press' })
  })
})
