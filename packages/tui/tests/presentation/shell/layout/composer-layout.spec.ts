...
describe('ComposerAnchoredLayout', () => {
  ...
  it('clips an unbounded readable surface to the terminal viewport', () => {
    const layout = new ComposerAnchoredLayout(
      new Text('header', 0, 0),
      new Text('message', 0, 0),
      new Text('status', 0, 0),
    ...
    layout.setActiveSurface({
      kind: 'readable',
      component: new Text(Array.from({ length: 80 }, (_, index) => `row ${String(index + 1)}`).join('\n'), 0, 0),
    })

    expect(layout.render(80)).toHaveLength(24)
  })

  it('pages through every row of an overflowing readable surface', () => {
    const layout = new ComposerAnchoredLayout(
      new Text('header', 0, 0),
      new Text('message', 0, 0),
      new Text('status', 0, 0),
    ...
    layout.setActiveSurface({
      kind: 'readable',
      component: new Text(Array.from({ length: 20 }, (_, index) => `row ${String(index + 1)}`).join('\n'), 0, 0),
    })

    expect(layout.render(80).map(stripTerminalSequences).join('\n')).toContain('row 1')
    expect(layout.pageSurface(1)).toBe(true)
    const middle = layout.render(80).map(stripTerminalSequences).join('\n')
    expect(middle).toContain('row 8')
    expect(middle).not.toMatch(/\brow 1\b/u)
    expect(layout.pageSurface(1)).toBe(true)
    const tail = layout.render(80).map(stripTerminalSequences).join('\n')
