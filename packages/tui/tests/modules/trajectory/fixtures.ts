...
export function timedTraceEvents(): RuntimeSessionSnapshot['events'] {
  const tool = (
    seq: number,
    callId: string,
    title: string,
...
export function state(
  ...
  const value = {
    binding: { phase: 'active' as const, sessionId: 'session-trajectory' as SessionSummary['sessionId'], epoch: 0 },
  ...
  return {
    ...
    execution: buildExecutionSnapshot({
      ...
      sessionRunning: value.runState !== 'idle',
    }),
  }
}
