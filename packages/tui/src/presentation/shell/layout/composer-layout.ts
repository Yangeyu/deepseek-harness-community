...
export class ComposerAnchoredLayout extends Container {
  ...
  override render(width: number): string[] {
    ...
    const composer = this.renderComposer(width, viewportRows)
  ...
  /** Move an overflowing active Surface by rendered rows. */
  scrollSurface(delta: number): boolean {
    if (this.activeSurface === undefined || this.surfaceMaxTop === 0) return false
    const next = Math.max(0, Math.min(this.surfaceMaxTop, this.surfaceTop + delta))
    if (next === this.surfaceTop) return false
    this.surfaceTop = next
    return true
  }

  /** Move an overflowing active Surface by one viewport page. */
  pageSurface(direction: -1 | 1): boolean {
    return this.scrollSurface(direction * Math.max(1, this.surfacePageRows - 1))
  }
  ...
  private renderActiveSurface(width: number, surface: ActiveSurface, viewportRows: number): string[] {
    ...
    this.surfacePageRows = Math.max(1, viewportRows)
    this.surfaceMaxTop = Math.max(0, lines.length - viewportRows)
    this.surfaceTop = Math.min(this.surfaceTop, this.surfaceMaxTop)
    const visible = lines.slice(this.surfaceTop, this.surfaceTop + viewportRows)
