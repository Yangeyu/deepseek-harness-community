...
import type { RewindAction, RewindPlan } from '../index.ts'
...
export class RewindDialog implements SurfaceInputTarget {
  ...
  handleAction(action: SurfaceInputAction): void {
    ...
    if (action === 'surface.select-1' || action === 'surface.select-2'
      || action === 'surface.select-3' || action === 'surface.select-4') {
      this.selected = Number(action.slice('surface.select-'.length)) - 1
      return
    }
