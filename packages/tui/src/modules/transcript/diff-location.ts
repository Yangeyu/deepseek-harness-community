...
export interface DiffTextReader {
...
export class DiffLineLocator {
  ...
  constructor(private readonly files: DiffTextReader) {}
  ...
  resolve(state: Readonly<RuntimeSessionSnapshot>, onChange: () => void, signal?: AbortSignal): void {
    ...
    for (const entry of entries) {
      ...
      pending.push(Promise.all(diffs.map(async (diff): Promise<number | undefined> => {
        ...
        try {
          ...
          return locateHunkStart(await this.files.readText(path, signal), diff.newText)
        } catch {
          return undefined
        }
    ...
    void Promise.all(pending).then((resolved) => {
      if (signal?.aborted === true || capturedSessionId !== this.sessionId) return
