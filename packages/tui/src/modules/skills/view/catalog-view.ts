...
export class SkillsView implements SurfaceInputTarget {
  readonly inputContext = 'skills' as const
  private index = 0
  ...
  handleAction(action: SurfaceInputAction): void {
    ...
    if (action === 'surface.search') {
      this.onSearch(this.query)
      return
    }
    if (action === 'surface.create') {
      this.onCreate()
      return
    }
    if (action === 'surface.edit') {
      const selected = this.entries()[this.index]
      if (selected !== undefined) this.onEdit(selected.name)
      return
    }
    if (action === 'surface.refresh') {
      this.onRefresh()
      return
    }
